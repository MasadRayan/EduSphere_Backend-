...
const buildInvoiceBuffer = async (
	...
	doc
		.fillColor("#0f172a")
		.fontSize(22)
		.text("Course Registration Invoice", { align: "center" })
		.moveDown(0.5)
		.fontSize(10)
		.fillColor("#64748b")
	...
	doc
		...
		.fontSize(11)
		.fillColor("#475569")
		.text(`Course Code: ${payload.courseCode}`)
		.text(`Course Title: ${payload.courseTitle}`)
		.text(`Section: ${payload.sectionCode}`)
	...
	doc
		...
		.fillColor("#94a3b8")
		.text(
			"This is an auto-generated invoice for your course registration. For any queries, contact the registrar office.",
...
export const sendEnrollmentInvoice = async (payload: IInvoiceMailPayload) => {
	...
	const invoiceBuffer = await buildInvoiceBuffer(payload, html);

	await transporter.sendMail({
		from: config.email_sender,
		to: payload.to,
		subject: `Invoice for ${payload.courseTitle} - ${payload.courseCode} (${payload.sectionCode})`,
		html,
		attachments: [
			{
